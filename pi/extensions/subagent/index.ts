  ...
  pi.registerTool({
    ...
    renderResult(
      ...
      const renderCompact = (r: SingleResult, indent = "  "): string[] => {
        ...
        if (r.model) {
          lines.push(
            indent +
              ...
              " " +
              theme.fg("dim", r.model),
          );
        }
        if (r.promptContent) {
          lines.push(
            indent +
              theme.fg("mdHeading", "[Prompt]") +
              " " +
              theme.fg("dim", truncate(r.promptContent)),
          );
        }
        lines.push(
          indent +
            theme.fg("mdHeading", "[Task]") +
            " " +
            theme.fg("dim", truncate(r.task)),
        );
        ...
        const usageStr =
          r.usage.turns > 0 ? " (" + formatUsageStats(r.usage) + ")" : "";
        lines.push(
          indent +
            theme.fg("mdHeading", "[Status]") +
            " " +
            statusIcon +
            " " +
            statusText +
        ...
        if (finalOutput || toolCalls.length > 0) {
          const outputPreview = finalOutput
            ? truncate(finalOutput)
            : "(tool calls only)";
          lines.push(
            indent + theme.fg("mdHeading", "[Output]") + " " + outputPreview,
          );
        }
      ...
      const renderFull = (r: SingleResult, indent = "  "): string[] => {
        ...
        lines.push(
          indent + statusIcon + " " + statusText + theme.fg("dim", usageStr),
        );
          ...
          for (const item of displayItems) {
            if (item.type === "toolCall" && item.name && item.args) {
              lines.push(
                indent +
                  theme.fg("muted", "\u2192 ") +
                  formatToolCall(item.name, item.args, (c: string, t: string) =>
                    theme.fg(c as ThemeColor, t),
                  ),
              );
            }
          }
