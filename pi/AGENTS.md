...
- `extensions/rtk/` — RTK token 优化扩展，拦截 bash 命令改写为 rtk 等价命令以节省 token，全局可用
...
- 进行功能变更或修复前，先与用户讨论方案，确认后再动手写代码
- 简单明确的修改（如 typo、格式调整）可直接执行，无需讨论
- 讨论时列出可选方案、各自优劣，给出倾向性建议
...
- UI 组件从 `@earendil-works/pi-tui` 导入（`Container`、`Text`、`SelectList`、`Spacer` 等）
